...
define('DB_PORT',     '3306'); // Changed port 3306 while working
define('DB_USERNAME', 'root');
define('DB_PASSWORD', ''); //password was removed & put back
...
function updateProfileAdmin($username, $file)
{
    $picture = md5($username.time());
    $moved   = move_uploaded_file($file['picture']['tmp_name'], 'profiles/'.$picture);
...
function userUpdate($data)  //$data was inside the paranthesis
{
    session_start();
    ...
    $resetUsers = mysqli_query($link, $queryA);
    $rid = 0;
...
function adminUpdate($user, $pass) {
